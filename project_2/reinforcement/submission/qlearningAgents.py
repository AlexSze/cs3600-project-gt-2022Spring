...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        bestVal = self.qTable[(state, bestAction)]
        for action in actions:
            tempVal = self.qTable[(state, action)]
            if tempVal > bestVal:
                bestAction = action
                bestVal = tempVal
        return bestVal
    ...
    def update(self, state, action, nextState, reward):
        ...
        value = (1-self.alpha) * (self.qTable[(state, action)]) + self.alpha * (reward + self.discount * self.getValue(nextState))
