...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # guess the location of ghost
        ghostGuessPos = []
        for ghost in livingGhostPositionDistributions:
            maxProb = None
            maxPos = None
            for pos1, pos2 in ghost:
                if (maxProb == None or ghost[pos1, pos2] > maxProb):
                    maxProb = ghost[pos1, pos2]
                    maxPos = (pos1, pos2)
            ghostGuessPos.append(maxPos)

        # find the clostest path to the nearest ghost
        resultAction = None
        smallestDistacnce = None

        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            for targetPos in ghostGuessPos:
                distance = self.distancer.getDistance(
                    successorPosition, targetPos)
                if (smallestDistacnce == None or distance < smallestDistacnce):
                    resultAction = action
                    smallestDistacnce = distance
